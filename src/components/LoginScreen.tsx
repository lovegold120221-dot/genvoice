...
export function LoginScreen({ onLogin, isLoggingIn }: { onLogin: () => void, isLoggingIn: boolean }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#050505] text-white overflow-y-auto z-50">
      {/* Background glow */}
      <div 
      ...
      <div className="flex flex-col items-center justify-center min-h-full px-6 py-12 relative z-10 w-full max-w-sm mx-auto">
        {/* Logo */}
        ...
        <button 
          onClick={onLogin}
          disabled={isLoggingIn}
          className="w-full bg-[#0c0c0c] border border-[#222] text-white font-medium py-4 rounded-full text-[15px] hover:bg-[#181818] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-70 disabled:active:scale-100"
        >
          <div className="bg-white text-black rounded-full flex items-center justify-center w-6 h-6">
            <svg viewBox="0 0 24 24" className="w-[14px] h-[14px]" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
